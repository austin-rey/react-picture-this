    ...
    return (
        <div className="flex flex-col justify-between mx-6 md:flex-row">
            <div className="flex flex-col items-center w-full md:mr-5 md:text-left">
                ...
                <input type="search" id="setSearch" name="search" placeholder="Creator, Set, Picture Name..." className="w-full p-2 rounded-md border-4 border-4 border-gray-500 border-opacity-20 focus:border-green-500 outline-none h-full hover:border-green-500" onChange={searchChange} value={searchQuery}/>
            </div>
            ...
            <button onClick={openModal} className="w-full bg-green-700 text-white rounded-md p-4 font-bold mt-4 md:mt-0">Create Set</button>
        </div>
    )
}
